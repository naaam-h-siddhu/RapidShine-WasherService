...
public class RabbitMQConfig {
    ...
    // Review Service message
    // Review → Washer rating update
    public static final String REVIEW_WASHER_QUEUE = "review.washer.rating.update.queue";
    public static final String REVIEW_WASHER_EXCHANGE = "review.washer.rating.update.exchange";
    public static final String REVIEW_WASHER_ROUTING_KEY = "review.washer.rating.update";
    ...
    // Review → Washer beans
    @Bean
    public Queue reviewWasherQueue() {
        return new Queue(REVIEW_WASHER_QUEUE, true);
    }

    @Bean
    public TopicExchange reviewWasherExchange() {
        return new TopicExchange(REVIEW_WASHER_EXCHANGE);
    }

    @Bean
    public Binding reviewWasherBinding() {
        return BindingBuilder
                .bind(reviewWasherQueue())
                .to(reviewWasherExchange())
                .with(REVIEW_WASHER_ROUTING_KEY);
    }
